...
- `GET /todoist` returns flat JSON the board renders verbatim (same anchor-on-`now` contract as `/schedule`): `configured`, `filter_name`, `error`, and `tasks[]` (`id`, `content`, `priority` 1–4/4-highest, `project`, `labels` joined as `@a @b`, pre-formatted `due_label` in the configured tz, `overdue`). Shown on the board's Todoist tab (`src/ui/page_todoist.cpp`, `LV_SYMBOL_LIST` icon) as a **scrollable list of one saved filter's tasks**, sorted overdue→soonest→undated with priority as tiebreaker (server-side in `buildBoard`).
- Uses the **`@doist/todoist-sdk`** package (the renamed successor to the deprecated `@doist/todoist-api-typescript`). `apps/api/src/todoist.ts` calls `getTasksByFilter({query})` + `getProjects()` (for id→name) + `closeTask(id)`. The SDK has **no "list saved filters" call** in its task/project surface, so `listFilters` uses the SDK's `sync({resourceTypes:['filters'], syncToken:'*'})` (filters are a Sync-API resource, not REST — `GET /api/v1/filters` 404s). Only `activeFilterId` lives in gitignored `apps/api/data/todoist.json` (`todoistConfig.ts`); the active filter's name+query are resolved **live per fetch** so renames/edits in Todoist propagate.
- Auth is a **personal API token** (Todoist → Settings → Integrations → Developer) read from the **`TODOIST_TOKEN` env var** (`.env`), exactly like the Google secrets — *not* the admin UI. Unset = the tab is unconfigured. The admin **Todoist** tab (`apps/admin/src/pages/TodoistPage.vue`) only picks the active filter and shows a read-only "token set" indicator (`GET /api/todoist/config` returns `hasToken` + `activeFilterId`, never the token). Cached (`todoistCacheTtlMinutes`, default 5); board polls `/todoist` every 60 s; a transient (non-auth) error keeps the last-good task list instead of flapping to empty. Cache busts on `POST /api/sync` and after a completion.
- **Tap-to-complete is the only board→API write path in the repo.** The board's checkbox (the sole tap target per row; its ring carries the priority color) opens a ~4 s **Undo** toast; the timer + commit live on the board (`page_todoist.cpp`), and only on commit does `todoist_client.cpp` enqueue the id to its core-0 fetch task, which POSTs `/todoist/complete {id}` → SDK `closeTask` → cache bust → the follow-up poll drops it. A failed POST (502) brings the row back on the next poll (self-healing); no permanent client-side hide.
- `todoist_client.cpp` mirrors `claude_client.cpp` (core-0 fetch, mutex + fresh flag) plus a FreeRTOS queue for completion ids — never call LVGL from it.
